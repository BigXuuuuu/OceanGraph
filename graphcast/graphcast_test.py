...
- Dynamic edge weights on Grid2Mesh edges
"""
...
LEVEL_OCEAN = (
    0.494025, 2.645669, 5.078224, 7.929560,
...
class GraphCast(predictor_base.Predictor):
    """GraphCast Predictor with Patch-Grid2Mesh and dynamic edge weights.
...
#             output_grid_nodes, targets_template)
    ...
    def _init_grid2mesh_graph(self) -> typed_graph.TypedGraph:
        ...
        # ---------- 动态权重生成与拼接 ----------
        # edge_features_np 可能是 np.ndarray，这里转为 jnp
        edge_features = jnp.asarray(edge_features_np, dtype=jnp.float32)
        dynamic_weights = GraphCast.DynamicEdgeWeights(
            name="grid2mesh_dynamic")(edge_features)  # [num_edges, 1]
        edge_features = jnp.concatenate([edge_features, dynamic_weights], axis=-1)
    ...
    def _run_grid2mesh_gnn(self, grid_node_features: chex.Array,
        ...
        new_edges = edges._replace(
            features=_add_batch_second_axis(
                edges.features.astype(dummy_mesh_node_features.dtype),
                batch_size))
